...
$sth_g = $pdo->prepare("SELECT * FROM tb_summary WHERE location = 2 ORDER BY timestamp DESC LIMIT 24;"); //SELECT pm1, pm25, pm10, timestamp FROM tb_data ORDER BY timestamp DESC LIMIT 228;
$sth_g->execute();
        ...
        <div class="row mb-3">
            <div class="col-12 text-center">
                ...
                <div class="mt-2">
                    <!-- Current Location -->
                    <h4 class="d-inline">
                    Chemical Engineering Building @KMUTT. 
            ...
            $.ajax({
                ...
                success:function(data) {
                    if (data.status == 'ok') {
                        ...
                        pm25 = data.result_02.pm25; // Value of pm2.5 as microgram
				
						var current_02 = data.result_02;
                        $('#pm25').text(current_02.pm25);
                        $('#pm1').text(current_02.pm1);
                        $('#pm10').text(current_02.pm10);
                        $('#temp').text(current_02.temp);
                        $('#humi').text(current_02.humi);
                        $('#aqi_pm25').text(current_02.aqi_pm25);
                        $('#aqi_pm10').text(current_02.aqi_pm10);
                        $('#location').text(current_02.location);
                        $('#timestamp').text(current_02.timestamp);
